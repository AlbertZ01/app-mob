...
def draw_icon(size: int, foreground_only: bool = False) -> Image.Image:
    image = Image.new("RGBA", (size, size), (0, 0, 0, 0) if foreground_only else "#101820")
    draw = ImageDraw.Draw(image)

    if not foreground_only:
        for y in range(size):
            blend = y / size
            r = round(16 * (1 - blend) + 24 * blend)
            g = round(24 * (1 - blend) + 58 * blend)
            b = round(32 * (1 - blend) + 55 * blend)
...
def draw_splash() -> Image.Image:
    size = 1024
    image = Image.new("RGBA", (size, size), "#101820")
    ...
    for text, y, fnt, fill in [
        ("MoodMix", 700, title_font, "#F4F1EA"),
        ("AI + Spotify discovery", 805, subtitle_font, "#D9B44A"),
    ]:
